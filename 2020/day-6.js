...
function countUniqueCharacters(sequence = "") {
  return sequence.split("").filter(char => /[a-z]/.test(char)).reduce((holder, char) => {
...
function countSharedCharacters(group = []) {
  let sharedCharacters = group[0].split("");

  group.slice(1).forEach(person => {
    const personAnswers = person.split("");
    sharedCharacters = personAnswers.reduce((holder, letter) => {
      if (sharedCharacters.includes(letter)) {
        holder.push(letter);
      }
      return holder;
    }, []);
...
function sumAnswers(answers = "") {
  return answers.split("\n\n").reduce((sum, answer) => sum + countUniqueCharacters(answer), 0);
}

function sumSharedCharacters(answers = "") {
  return answers.split("\n\n").map(item => item.split("\n")).reduce((sum, answer) => sum + countSharedCharacters(answer), 0);
}

// const result = sumAnswers(input)
const result = sumSharedCharacters(input);
console.log(result);
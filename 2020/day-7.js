...
function parseRule(rule = "") {
  const colour = /(.+?) bags/.exec(rule)[1];
  const children = /contain (.+)/
    .exec(rule)[1]
    .split(",")
    .reduce((contents, bagRule) => {
      if (bagRule !== "no other bags.") {
        const count = parseInt(/\d+/.exec(bagRule)[0], 10);
        const colour = /\d (.+) bags?/.exec(bagRule)[1];
...
function parseAllRules(rules = "") {
  return rules.split("\n").map((rule) => parseRule(rule));
}

function testIfCanContain(
  bag = "",
  targetBag = "",
  rules = [
    {
  ...
  const matchingRule = rules.find((rule) => rule.colour === targetBag);
  return matchingRule.children.some((child) => {
    if (child.colour === bag && child.count > 0) {
      return true;
    } else {
      return testIfCanContain(bag, child.colour, rules);
    }
  });
}
...
function sumBags(
  ...
  const matchingRule = rules.find((rule) => rule.colour === bag);

  return matchingRule.children.reduce((sum, child) => {
    const childContents = child.count * sumBags(child.colour, rules);
    return sum + child.count + childContents;
  }, 0);
}

function sumPossibleBags(rulesString = "", targetBag = "") {
  const parsedRules = parseAllRules(rulesString);
  return parsedRules.filter((rule) =>
    testIfCanContain(targetBag, rule.colour, parsedRules)
  ).length;
}

function sumContents(rulesString = "", targetBag = "shiny gold") {
  const parsedRules = parseAllRules(rulesString);
  return sumBags(targetBag, parsedRules);
}

const result = sumContents(input, "shiny gold");
console.log(result);

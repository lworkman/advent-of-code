...
function oldPasswordPolicy(passwordAndRule = "") {
  const {
    rule,
    password,
    minimum,
    max,
    passwordLetter
  } = getPasswordDetails(passwordAndRule);

  const count = (password.match(new RegExp(passwordLetter, "g")) || []).length;

  return count >= minimum && count <= max;
}

function newPasswordPolicy(passwordAndRule = "") {
  const {
    rule,
    password,
    minimum,
  ...
  const minValid = password[minimum - 1] === passwordLetter;
  const maxValid = password[max - 1] === passwordLetter;

  return ((minValid && !maxValid) || maxValid && !minValid)
}

function getPasswordDetails(passwordAndRule = "") {
  const [rule, password] = passwordAndRule.split(":").map(item => item.trim());
  const [minimum, max] = rule.split("-").map(num => parseInt(num, 10));
  const passwordLetter = /([a-z])/.exec(rule)[1];
...
function countValidPasswords(passwords = []) {
  return passwords.filter(password => newPasswordPolicy(password)).length;
}

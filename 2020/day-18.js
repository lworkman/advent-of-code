...
function processEquation(line = "") {
  let editedLine = line;
  let pStart = undefined;
  let pContain = 0;
  let parens = [];
  editedLine.split("").forEach((char, index) => {
    switch (char) {
      case "(":
        if (pStart !== undefined) {
          pContain++;
  ...
  parens.forEach((set) => {
    const original = line.slice(set[0], set[1] + 1);
    const value = processEquation(original.slice(1, -1));
    const currentIndex = editedLine.indexOf(original);
    editedLine =
      editedLine.slice(0, currentIndex) +
      value +
      editedLine.slice(currentIndex + original.length);
  });

  const start = parseInt(/\d+/.exec(editedLine), 10);
  const instructions = editedLine.match(/[+*] \d+/g);

  return instructions.reduce((sum, action) => {
    const value = parseInt(/\d+/.exec(action));
    switch (action[0]) {
      case "*":
        return sum * value;
      case "+":
...
function processEquationRound2(line = "") {
  let editedLine = line;
  let pStart = undefined;
  let pContain = 0;
  let parens = [];
  ...
  parens.forEach((set) => {
    const original = line.slice(set[0], set[1] + 1);
    const value = processEquationRound2(original.slice(1, -1));
    const currentIndex = editedLine.indexOf(original);
    editedLine =
      editedLine.slice(0, currentIndex) +
      value +
  ...
  while (editedLine.includes("+") && i < limit) {
    i++;
    const match = /(\d+) \+ (\d+)/.exec(editedLine);
    const value = parseInt(match[1], 10) + parseInt(match[2], 10);
    editedLine =
      editedLine.slice(0, match.index) +
      value +
      editedLine.slice(match.index + match[0].length);
  }
...
function sumEquations(raw = "") {
  const equations = raw.split("\n");

  return equations.reduce((sum, e) => {
    return sum + processEquationRound2(e);
  }, 0);
}

const result = sumEquations(input);
console.log(result);

...
function buildChart(lines = [""]) {
  const chart = lines.map((line) => line.split(""));

  return chart;
}

function performRound(chart = [[""]], limit = 4, useAxis = false) {
  const output = [];

  chart.forEach((row, y) => {
    const newRow = [...row];
    row.forEach((spot, x) => {
      const coordiante = [x, y];
      const surroundingSpaces = useAxis
        ? getSpotsOnAxis(coordiante, chart)
        : getSurroundingSeats(coordiante, chart);
      ...
      switch (spot) {
        case "L": {
          if (surroundingSpaces.every((space) => space !== "#")) {
            newRow[x] = "#";
          } else {
            newRow[x] = spot;
          }
        ...
        case "#": {
          if (
            surroundingSpaces.filter((space) => space === "#").length >= limit
          ) {
            newRow[x] = "L";
          } else {
            newRow[x] = spot;
...
function getSurroundingSeats(coordinate = [0, 0], chart = [[""]]) {
  const output = [];
  const [x, y] = coordinate;

  if (y > 0) {
    output.push(chart[y - 1][x]);
    output.push(chart[y - 1][x + 1]);
    output.push(chart[y - 1][x - 1]);
  }
  ...
  output.push(chart[y][x + 1]);
  output.push(chart[y][x - 1]);

  return output;
}

function countSeats(chart = [[""]]) {
  return chart
    .map((row) => row.join(""))
    .join("")
    .replace(/(\.|L)/g, "").length;
...
function compareChart(a = [[""]], b = [[""]]) {
  return (
    a.map((x) => x.join("")).join("") === b.map((x) => x.join("")).join("")
  );
}

function runRoundsTillNoChange(lines = [""]) {
  let chart = buildChart(lines);
  const limit = 1000;
  let count = 0;

  while (true && count < limit) {
    count++;
    const newChart = performRound(chart);

    if (compareChart(newChart, chart)) {
      break;
    } else {
      chart = newChart;
    }
  ...
  return countSeats(chart);
}

function getSpotsOnAxis(coordinate = [0, 0], chart = [[""]]) {
  const output = [];
  const movements = [
    [0, -1],
    [0, 1],
  ...
  const shouldLog = y === 1 && x === 0;

  movements.forEach((move) => {
    let seat;
    let yAdjustment = move[0] + y;
    let xAdjustment = move[1] + x;
    while (!seat) {
      if (
        yAdjustment > chart.length - 1 ||
        yAdjustment < 0 ||
        xAdjustment > chart[0].length - 1 ||
        xAdjustment < 0
      ) {
        break;
      }
      const spot = chart[yAdjustment][xAdjustment];

      if (spot !== ".") {
        if (shouldLog) {
          console.log(spot, yAdjustment, xAdjustment);
        }
        seat = spot;
      }
...
function runRoundsTillNoChangeProblem2(lines = [""]) {
  let chart = buildChart(lines);
  const limit = 100;
  let count = 0;
  ...
  while (true && count < limit) {
    count++;
    const newChart = performRound(chart, 5, true);

    console.log(newChart.map((row) => row.join("")));
    if (compareChart(newChart, chart)) {
      break;
    } else {
      chart = newChart;
...
const result = runRoundsTillNoChangeProblem2(input);
console.log(result);

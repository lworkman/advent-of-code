...
const input = `pnglkx nfnzx tjsdp jkbqk rpmqq gzgvdh rgdx szsbj xjdhk zfml ddbmq thvm mvnqdh gsgmdn dtlhh rqqfnlc bxv nthhxn hnmjfl ckqq qrsczjv fkh hkxcb rpcdfph flhfddq qspfqb zmb rpmmv zrgzf jfqqgtl xxfgvz kltcm xjrpr vnfmc xhmmt zkzdrn jmdg xgbvk ngqh hlmvqh djpsmd bnzq rbvdt tfmgl pjln (contains sesame, nuts)
...
function readLine(line = "") {
  const ingredients = /(.+)\(/.exec(line)[1].trim().split(" ");
  const caughtAllergens = /\(contains (.+)\)/.exec(line);
  let allergens = [];

  if (caughtAllergens) {
    allergens = caughtAllergens[1]
      .split(",")
      .map((allergens) => allergens.trim());
  }

  return allergens.reduce((holder, allergen) => {
    holder.push({
      allergen,
      ingredients,
    });
...
function countWordOccurance(raw = "") {
...
function findGoodIngredients(raw = "") {
  const lines = raw.split("\n");

  const allergenWordMap = lines.reduce((holder, line) => {
    const allergens = readLine(line);
    allergens.forEach(({allergen, ingredients}) => {
      if (!holder[allergen]) {
        holder[allergen] = [];
      }
      holder[allergen].push(ingredients);
    })
    return holder;
  },{});
  const wordCount = countWordOccurance(raw);

  let allergiesLeft = Object.keys(allergenWordMap);
  const mappedIngredients = {};

  while(allergiesLeft.length > 0 ) {

    const known = [];

    allergiesLeft.forEach(key => {
      const match = checkAllergen(key, allergenWordMap[key], mappedIngredients);

      if (match) {
        mappedIngredients[match] = key;
        known.push(key);
      }
    });

    allergiesLeft = allergiesLeft.filter(allergy => !known.includes(allergy));
  }

  console.log(mappedIngredients);
  const badIngredients = Object.keys(mappedIngredients);

  return badIngredients.sort((a, b) => mappedIngredients[a] < mappedIngredients[b] ? -1 : 1).join(",");
  
}

function checkAllergen(allergen = "", ingredients = [[""]], knownIngredients = {}) {
  let possibleIngredients = ingredients[0].filter(element => !knownIngredients[element]);

  ingredients.slice(1).forEach(list => {
    let latestRound = [];
    list.forEach(element => {
      if (possibleIngredients.includes(element)) {
        latestRound.push(element);
      }
    });
    possibleIngredients = latestRound;
  });

  if (possibleIngredients.length === 1) {
    return possibleIngredients[0];
  }
}

const result = findGoodIngredients(input);
console.log(result);

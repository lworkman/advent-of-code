...
function testPassport(passport) {
  const requiredFields = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"];

  return requiredFields.every((field) =>
    new RegExp(`${field}:`).test(passport)
  );
}

function strictTestPassport(passport = "") {
  const rules = [
    {
      id: "byr",
      regexp: /^(19[2-9][0-9]|200[0-2])$/,
    },
    {
      id: "iyr",
      regexp: /^(201[0-9]|2020)$/,
    ...
    {
      id: "hgt",
      regexp: /^(1[5-8][0-9]cm|19[0-3]cm|59in|6[0-9]in|7[0-6]in)$/,
    },
    {
      id: "hcl",
      regexp: /^#[0-9a-f]{6}$/,
  ...
  return rules.every((rule) => {
    if (!passport.includes(`${rule.id}:`)) {
      return false;
    }

    const search = new RegExp(`${rule.id}:(.+?)(\n| |$)`).exec(passport);

    if (!search) {
      console.log(passport, rule);
      return false;
    }
    const entry = search[1];

    return rule.regexp.test(entry);
  });
}

function testAllPassports(batch = "") {
  const passports = batch
    .split(/[\r\n]{2}/)
    .map((item) => item.replace(/[\r\n]/, " "));

    console.log(passports);

  // return passports.filter((passport) => testPassport(passport)).length;
  return passports.filter((passport) => strictTestPassport(passport)).length;
}

console.log(testAllPassports(input));

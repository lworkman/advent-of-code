...
const input = `mask = 0111X10100100X1111X10010X000X1000001
mem[50907] = 468673978
mem[22295] = 3337449
mem[58474] = 56418393
mem[15362] = 243184
...
function applyMask(mask = "", value = "") {
  const bitValue = parseInt(value).toString(2);
  let fullSizedValue = "";

  for (let i = bitValue.length; i < mask.length; i++) {
    fullSizedValue = fullSizedValue + "0";
  }

  fullSizedValue = fullSizedValue + bitValue;

  mask.split("").forEach((char, index) => {
    if (char !== "X") {
      fullSizedValue =
        fullSizedValue.slice(0, index) + char + fullSizedValue.slice(index + 1);
    }
  });

  return parseInt(fullSizedValue, 2);
}

function applyProgram(input = "") {
  const instructions = input.split("\n").map((line) => {
    if (line.includes("mask")) {
      return {
        action: "mask",
        value: /mask = (.+)/.exec(line)[1],
      };
    } else {
      return {
        action: "set",
        address: /mem\[(.+)\]/.exec(line)[1],
        value: /= (.+)/.exec(line)[1],
      };
    }
  });
  const store = instructions.reduce(
    (store, instruction) => {
      switch (instruction.action) {
        case "mask":
          store.currentMask = instruction.value;
          break;
        case "set":
          store.memory[instruction.address] = applyMask(
            store.currentMask,
            instruction.value
          );
          break;
  ...
  return store.memory.reduce((sum, value) => sum + value, 0);
}

function generateVariations(original = "", details = []) {
  function createVariations(index = 3, transformations = []) {
    let output = [];
    const ownTransformation = (value) => {
      const spot = details[index];
      return value.slice(0, spot) + "1" + value.slice(spot + 1);
    };
    const ownTransformation2 = (value) => {
      const spot = details[index];
      return value.slice(0, spot) + "0" + value.slice(spot + 1);
    };
    output.push(ownTransformation);
    output.push(ownTransformation2);

    transformations.forEach((t) => {
      output.push((value) => t(ownTransformation(value)));
      output.push((value) => t(ownTransformation2(value)));
    });

    if (index !== 0) {
      return createVariations(index - 1, output);
    } else {
      return output;
    }
  }

  return createVariations(details.length - 1, [])
    .map((action) => action(original))
    .reduce((holder, v) => {
      if (!holder.includes(v)) {
        holder.push(v);
      }
      return holder;
    }, [])
...
function getAddresses(mask = "", value = "") {
  const bitValue = parseInt(value).toString(2);
  let fullSizedValue = bitValue;

  for (let i = bitValue.length; i < mask.length; i++) {
    fullSizedValue = "0" + fullSizedValue;
  }

  const alterations = mask.split("").map((char, index) => {
    return {
      char,
      index,
    };
  });
  const modifiedValue = alterations.reduce((holder, line) => {
    switch (line.char) {
      case "1":
        holder =
          holder.slice(0, line.index) + "1" + holder.slice(line.index + 1);
        break;
      case "X":
        holder =
          holder.slice(0, line.index) + "0" + holder.slice(line.index + 1);
    }
    return holder;
  }, fullSizedValue);

  const floatActions = alterations
    .filter((action) => action.char === "X")
    .map((action) => action.index);

  return generateVariations(modifiedValue, floatActions).map((address) =>
    parseInt(address, 2)
  );
}

function secondProgram(input = "") {
  const instructions = input.split("\n").map((line) => {
    if (line.includes("mask")) {
      return {
        action: "mask",
      ...
      switch (instruction.action) {
        ...
        case "set":
          ...
          addresses.forEach((address) => {
            totalActions++;
            store.memory[address] = parseInt(instruction.value, 10);
          });
          break;
      }
  ...
  return Object.values(store.memory).reduce((sum, value) => sum + value, 0);
}

const result = secondProgram(input);
// 4640050608017 too low
console.log(result);

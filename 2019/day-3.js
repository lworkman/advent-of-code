...
const test2 = `R75,D30,R83,U83,L12,D49,R71,U7,L72
U62,R66,U55,R34,D71,R55,D58,R83`;

function runWire(path = [""]) {
  let x = 0;
  let y = 0;
  const output = [[x, y]];
...
function runWireWithSteps(path = [""]) {
  let x = 0;
  let y = 0;
  let steps = 0;
  const output = [[x, y, steps]];
  ...
  path.forEach((direction) => {
    ...
    steps += value;
    output.push([x, y, steps]);
  });

  return output;
}

function findClosestPoint(raw = "") {
  const [wire1, wire2] = raw.split("\n").map((row) => row.split(","));

  const wire1Points = runWire(wire1);
  const wire2Points = runWire(wire2);
  // const wire1Points = [
  //   [0, 0],
  //   [0, 5],
  //   [5, 5],
  ...
  for (let i = 1; i < wire1Points.length; i++) {
    const [startX, startY] = wire1Points[i - 1];
    const [endX, endY] = wire1Points[i];
    wire1Checks.push(([x, y] = [0, 0], [x2, y2] = [0, 0]) => {
      if (startY === endY && x === x2) {
        const minX = Math.min(startX, endX, x, x2);
        const maxX = Math.max(startX, endX, x, x2);
        const minY = Math.min(y, y2, startY, endY);
        const maxY = Math.max(y, y2, startY, endY);
        if (
          (minX === startX || minX === endX) &&
        ...
        if (
          (minX === x || minX === x2) &&
          (maxX === x || maxX === x2) &&
          (minY === startY || minY === endY) &&
          (maxY === startY || maxY === endY) &&
          y !== 0 &&
          startX !== 0
        ) {
          return [startX, y];
  ...
  for (let i = 1; i < wire2Points.length; i++) {
    wire1Checks.forEach((check) => {
      const point = check(wire2Points[i - 1], wire2Points[i]);
      if (point) {
  ...
  return intersectionPoints.map(([x,y]) => Math.abs(x) + Math.abs(y)).sort((a,b) => a - b)[0]
}

function findLeastStepsPoint(raw = "") {
  const [wire1, wire2] = raw.split("\n").map((row) => row.split(","));

  const wire1Points = runWireWithSteps(wire1);
  const wire2Points = runWireWithSteps(wire2);
  const wire1Checks = [];

  for (let i = 1; i < wire1Points.length; i++) {
    const [startX, startY, startStep] = wire1Points[i - 1];
    const [endX, endY, endStep] = wire1Points[i];
    wire1Checks.push(([x, y, step] = [0, 0], [x2, y2, step2] = [0, 0]) => {
      if (startY === endY && x === x2) {
        const minX = Math.min(startX, endX, x, x2);
        const maxX = Math.max(startX, endX, x, x2);
        const minY = Math.min(y, y2, startY, endY);
        ...
        if (
          ...
          const line1Step = Math.abs(startX - x) + startStep;
          const line2Step = Math.abs(startY - y) + step;
          return [x, startY, line2Step + line1Step];
        }
      } else if (startX === endX && y === y2) {
        const minX = Math.min(startX, endX, x, x2);
        const maxX = Math.max(startX, endX, x, x2);
        ...
        if (
          ...
          const line1Step = Math.abs(startY - y) + startStep;
          const line2Step = Math.abs(startX - x) + step;
          return [startX, y, line2Step + line1Step];
        }
      }
    });
  }
  ...
  return intersectionPoints.map(([x,y, step]) => step).sort((a,b) => a - b)[0]
}

const result = findLeastStepsPoint(input);

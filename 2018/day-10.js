...
const input = `position=<-31138, -10302> velocity=< 3,  1>
...
function convertToPoints(raw = "") {
  return raw.split("\n").map((point) => {
    const result = /position=<(.\d+),(.?.?\d+).+=<(.\d+),(.?.?\d+)/.exec(point);
    const x = Number(result[1].trim());
    const y = Number(result[2].trim());
    const velocityX = Number(result[3].trim());
    const velocityY = Number(result[4].trim());
...
function runRound(points = [], round = 0) {
  let lowY = 99999;
  let lowX = 99999;
  let maxX = 0;
  let maxY = 0;
  let newPoints = points.map((point) => {
    point.position[0] += point.velocityX;
    point.position[1] += point.velocityY;
    if (point.position[0] < lowX) {
      lowX = point.position[0];
    }
    if (point.position[1] < lowY) {
      lowY = point.position[1];
    }
    if (point.position[0] > maxX) {
      maxX = point.position[0];
    }
    if (point.position[1] > maxY) {
      maxY = point.position[1];
    }
  ...
  if (maxY - lowY < 100 && maxX - lowX < 100) {
    console.log(`Round: ${round}`);
    for (let y = lowY; y <= maxY; y++) {
      map[y] = [];
      for (let x = lowX; x <= maxX; x++) {
        map[y][x] = ".";
      }
    }

    newPoints.forEach((point) => {
      const newY = point.position[1];
      const newX = point.position[0];
...
function figureOutMessage(raw = "") {
  let points = convertToPoints(raw);
  ...
  while (i < limit) {
    ...
    points = runRound(points, i);
    i++;
  }
}

const result = figureOutMessage(input);

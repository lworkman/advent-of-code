...
const input = `[1518-11-13 00:04] Guard #2411 begins shift
...
/**
 * Read instructions and order them chronologically
 * @param {*} raw
 */
function readInstructions(raw = "") {
  const output = raw
    .split("\n")
    .map((code) => {
      const regexMatch = /\[(\d\d\d\d-\d\d-\d\d) (\d\d):(\d\d)\](.+)/.exec(
        code
      );
      const date = regexMatch[1];
      const hour = parseInt(regexMatch[2]);
      const minute = parseInt(regexMatch[3]);
      const action = regexMatch[4].trim();
      return {
        date,
        hour,
        minute,
    ...
    .reduce((holder, action) => {
      let dateToUse = action.date;
      if (action.hour === 23) {
        const regexMatch = /(\d\d\d\d)-(\d\d)-(\d\d)/.exec(action.date);
        let year = parseInt(regexMatch[1]);
        let month = parseInt(regexMatch[2]);
        let date = parseInt(regexMatch[3]);
        if (date + 1 === 31) {
          switch (month) {
            case 2:
            case 4:
          ...
          if (month !== 12) {
            ...
            throw new Error("AAAHHH");
          }
        } else {
          date ++;
        }
        dateToUse = `${year}-${prependZero(month)}-${prependZero(date)}`;
      }
      if (!holder[dateToUse]) {
        holder[dateToUse] = [];
      }
      holder[dateToUse].push(action);
      return holder;
    }, {});
...
function figureOutMinutes(raw = "") {
  const instructions = readInstructions(raw);
  const guardRecord = {};

  Object.keys(instructions).forEach((key) => {
    const set = instructions[key];
    const guardId = /Guard #(\d+)/.exec(set[0].action)[1];
    if (!guardRecord[guardId]) {
      guardRecord[guardId] = {
        total: 0,
        days: [],
        minutes: {},
    ...
    const nonGuardActions = set.slice(1);
    const sets = [];

    for (let i = 0; i < nonGuardActions.length; i++) {
      const setter = Math.floor(i / 2);
      if (!sets[setter]) {
        sets[setter] = [];
      }

      sets[setter].push(nonGuardActions[i]);
    }
    ...
    sets.forEach(([start, end]) => {
      ...
      const tempKey = `${currentHour}:${currentMinute}`;

      if (!guardRecord[guardId].minutes[tempKey]) {
        guardRecord[guardId].minutes[tempKey] = 0;
      }

      guardRecord[guardId].minutes[tempKey]++;
      ...
      guardRecord[guardId].total += minutes;
    });
    guardRecord[guardId].days = nonGuardActions;
  });

  const chosenGuard = Object.keys(guardRecord).map(key => guardRecord[key]).sort((a,b) => {
    return b.total - a.total
  })[0];

  const foundMinutes = chosenGuard.minutes;
  const chosenMinute = Object.keys(foundMinutes).sort((keyA, keyB) => {
    const a = foundMinutes[keyA];
    const b = foundMinutes[keyB];

    return b - a;
  })[0];

  console.log(chosenGuard);

  return Number(chosenMinute.slice(2)) * Number(chosenGuard.id);
}

function findMostCommonMinute(raw = "") {
  const instructions = readInstructions(raw);
  const guardRecord = {};
  ...
  const found = Object.keys(guardRecord).map(key => guardRecord[key]).map(guard => {
    let highestMinute
    let record = 0;

    Object.keys(guard.minutes).forEach(key => {
      const count = guard.minutes[key];
      if (count > record) {
        record = count;
        highestMinute = key;
      }
    ...
    guard.highestMinute = highestMinute;
    guard.highestMinuteCount =record;

    return guard;
  }).sort((a, b) => b.highestMinuteCount - a.highestMinuteCount)[0];

  return Number(found.highestMinute.slice(2)) * Number(found.id);
}

const result = findMostCommonMinute(input);
